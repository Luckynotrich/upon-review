...
export default function ImgMediaCard({ category, review, toggleItem, elem }) {
  ...
  const [w,setW] = useState(dubya)
  const [f,setf] = useState(eff(elem))
...
useEffect(() => {
  ...
  let gridHeight =
    40 * (review.pros.length > review.cons.length
      ? review.pros.length
      : review.cons.length);
  let textHeight = Text?txthite(Text.length,f, w): 40;
  ...
  return (
    ...
    >
      ...
      <CardContent>
        ...
        >
          <Typography gutterBottom variant="p" component="div" sx={{ display: 'flex', flexFlow: 'row nowrap' }}>
          
              <div>{kategori}:</div>
              <h3 style={{marginLeft:"-25%",marginTop:"-2%"}}>
              {caption}
              {/* </h3>
 ...
 //function to dirive aproximate font size 
  ...
  const eff = (elem) => {
    let arr = (window.getComputedStyle(document.getElementById(elem)).fontSize).split('p')
    return arr[0]
    }
